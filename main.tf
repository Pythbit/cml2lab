...
resource "cml2_link" "lab_connections" {
  ...
  slot_a = strcontains(each.value.a_terminations[0].object.name, "Ethernet") ? reverse(split("/",each.value.a_terminations[0].object.name))[0] : "0"
  node_b = resource.cml2_node.lab_devices[each.value.b_terminations[0].object.device.name].id
  slot_b = strcontains(each.value.b_terminations[0].object.name, "Ethernet") ? reverse(split("/",each.value.b_terminations[0].object.name))[0] : "0"
}